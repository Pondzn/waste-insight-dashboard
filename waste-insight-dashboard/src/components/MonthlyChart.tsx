...
interface Props {
  monthlyRows:  WasteRow[]  // ALL year+dept filtered rows (all months)
  detailRows:   WasteRow[]  // for job count (also all months)
  ddMonth:      number | null   // dropdown single-month selection
  chartMonths:  number[]        // chart-click month selection
...
export function MonthlyChart({ monthlyRows, detailRows, ddMonth, chartMonths, onClickMonth }: Props) {
  ...
  // Dim logic: month is gray if there IS a selection AND this month is not selected
  ...
  return (
    <div className="card p-4 min-h-[300px]">
      ...
      <ResponsiveContainer width="100%" height={260}>
        <ComposedChart data={data} margin={{ top: 18, right: 48, bottom: 48, left: 8 }} style={{ cursor: 'pointer' }}
          onClick={(d, e) => {
        ...
        >
          ...
          <Line yAxisId="v" dataKey="target"  name="Target"   stroke="#2563eb" strokeWidth={2} strokeDasharray="5 3" dot={false} type="monotone" legendType="line"/>
          <Line yAxisId="v" dataKey="prevAvg" name="Prev Avg" stroke="#a78bfa" strokeWidth={1.5} strokeDasharray="2 3" dot={false} type="monotone" legendType="line"/>
          <Area yAxisId="c" dataKey="count" name="Jobs" type="monotone"
            ...
            activeDot={{ r: 5, fill: '#7c3aed' }} legendType="circle"
          />
        </ComposedChart>
      </ResponsiveContainer>
