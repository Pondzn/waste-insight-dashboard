...
export function AddpaperPage({ actioned }: { actioned: UseActionedJobsResult }) {
  ...
  const kpi = useMemo(() => {
    ...
    return { totalValue, totalTarget, achPct, totalJobs, bigJobs, bigVal, vsPrevPct, totalSales, wasteRate }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 lg:grid-cols-4 gap-3" style={{ minHeight: 100 }}>
        {anyLoading ? [0,1,2].map(i=><Sk key={i} h="h-24"/>) : (<>
          <KpiCard label="Total Value (THB)" value={fmtK(kpi.totalValue)}
            sub1={kpi.totalTarget > 0 && kpi.achPct !== null
              ...
              : undefined}
            sub2={kpi.vsPrevPct !== null ? `${Math.abs(kpi.vsPrevPct).toFixed(1)}% vs prev month` : undefined}
